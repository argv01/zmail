...
$Header: /usr/cvsroot/zmail/general/regexpr.c,v 2.20 2005/05/31 07:36:40 syd Exp $
...
static char hex_char_to_decimal(ch)
char ch;
{
  if (ch >= '0' && ch <= '9')
    return ch - '0';
...
#define ALLOC(amount)				\
  MACRO_BEGIN					\
    if (pattern_offset+(amount) > alloc)	\
      {						\
	alloc += 256 + (amount);		\
	pattern = (char *) realloc(pattern, (size_t) alloc); \
	if (!pattern)				\
	  goto out_of_memory;			\
      }						\
  MACRO_END

#define STORE(ch) pattern[pattern_offset++] = (char) (ch)
...
#define PUT_ADDR(offset,addr)				\
  MACRO_BEGIN						\
    ...
    pattern[(offset)] = (char) (disp & 0xff);            \
    pattern[(offset)+1] = (char) ((disp>>8) & 0xff);     \
  MACRO_END
...
#define ANSI_TRANSLATE(ch)				\
  ...
  if (alloc == 0 || pattern == NULL)
    {
      alloc = 256;
      /* Bart: Wed Mar 25 14:53:15 PST 1992
       * This is a potential memory leak if the caller does not hang on
       * to the structure pointed at by bufp.  re_destroy_pattern() ?
       */
      pattern = malloc((size_t) alloc);
      if (!pattern)
	goto out_of_memory;
    }
  pattern_offset = 0;
 ...
 op_error:
  SET_FIELDS;
  return catgets( catalog, CAT_GENERAL, 13, "Badly placed special character" );

 bad_match_register:
  SET_FIELDS;
  return catgets( catalog, CAT_GENERAL, 14, "Bad match register number" );

 hex_error:
  SET_FIELDS;
  return catgets( catalog, CAT_GENERAL, 15, "Bad hexadecimal number" );

 parenthesis_error:
  SET_FIELDS;
  return catgets( catalog, CAT_GENERAL, 16, "Badly placed parenthesis" );

 out_of_memory:
  SET_FIELDS;
  return catgets( catalog, CAT_GENERAL, 17, "Out of memory" );

 ends_prematurely:
  SET_FIELDS;
  return catgets( catalog, CAT_GENERAL, 18, "Regular expression ends prematurely" );

 too_complex:
  SET_FIELDS;
  return catgets( catalog, CAT_GENERAL, 19, "Regular expression too complex" );
}
#undef CHARAT
#undef NEXTCHAR
#undef GETHEX
...
#else
# define MAX_FAILURES     2700  /* max # of failure points before failing */
#endif /* !WIN16 */
...
regexp_registers_t regs;
{
  ...
  const char *text, *textend, *partend, *part_2_end;
	...
	case Cstar_jump:
	  ...
	  loop_p1:
		...
		switch (*p1++)
		  ...
		  default:
#if 0
		    printf(catgets( catalog, CAT_GENERAL, 20, "regexpr.c: processing star_jump: unknown op %d\n" ), p1[-1]);
#endif
		    break;
		  }
	      }
	...
	case Cfailure_jump:
	  if (failure_sp == failure_stack_end)
		...
		goto error;
	      ...
	      malloc((size_t) (MAX_FAILURES * sizeof(struct failure_point)));
	      failure_stack_end = failure_stack_start + MAX_FAILURES;
	      bcopy((char *)initial_failure_stack, (char *)failure_stack_start,
...
/*	  if (size2 == 0 ? text == string1 + size1 : text == string2 + size2) */
	  if (size2 == 0) {
	      if (text == string1 + size1) {
		  break;
	      }
	  } else if (text == string2 + size2) {	
		  break;
	      }
	  goto fail;
	case Cwordbeg:
...
/* This is #if'd out because some compilers complain the statement is
 * unreachable.  How annoying. */
#if 0
  abort();
  /*NOTREACHED*/
#endif /* 0 */
...
regexp_registers_t regs;
{
  char *fastmap, *translate;
  const char *text, *partstart, *partend;
  int dir, ret;
  char anchor;
  ...
  for (; range >= 0; range--, pos += dir)
    {
      /* Bart: Wed Mar 25 12:27:42 PST 1992
       * If mstop is going to cause an assertion failure, what's the point
       * of having it?  I don't know if this is how it's supposed to work,
       * but it gives the behavior we want it to have for wrapping search.
       */
      if (pos > mstop)
        {
	  mstop += size2;
...
const char *s;
{
  if (s == NULL)
	...
	return catgets( catalog, CAT_GENERAL, 21, "Out of memory" );
      return NULL;
    }
  if (!re_comp_buf.buffer)
    {
