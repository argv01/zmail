...
char *re_compile_pattern(const char *regex, int regex_size, regexp_t compiled);
...
/* This tries to match the regexp to the concatenation of string1 and
   ...
   overflow) is encountered.  mstop specifies the offset beyond
...
int re_search(regexp_t compiled, const char *string, int size,
...
/* This rearches for a substring matching the regexp.  This returns the first
   ...
   values indicate searching backwards. */
...
/* This is like re_search, but search from the concatenation of string1 and
   string2.  mstop specifies the offset beyond which a match must not go. */
