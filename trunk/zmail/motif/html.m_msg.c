/* This is a version of m_msg.c that uses the gnu HTML widget to display HTML */

/* m_msg.c     Copyright 1990 Z-Code Software Corp. */

#ifndef lint
static char	m_msg_rcsid[] =
    "$Id: html.m_msg.c,v 2.2 2005/05/09 09:15:20 syd Exp $";
#endif
...
struct phone_tag_parse_struct phone_tag_parse_text[] = {
    "To:",        0,
    "From:",      1,
    "Of:",        2,
    "Date:",      5,
...
struct phone_tag_parse_struct phone_tag_parse_checkbox[] = {
    "Phoned:",           0,
    "Call back:",        1,
    "Returned Call:",    2,
    "Wants to see you:", 3,
...
void
RebuildMsgDisplay(frame,initial)
ZmFrame frame;
int initial;
{
    msg_data *m_data;
    ...
    viewing_html = content_is_html(msg_no);
    ...
    if (viewing_html)
      {
        ...
        lockout = True;
        html_detach_images(frame,m_data->text_w,m_data);
        lockout = False;
        timeout_cursors(False);
      }
    if (viewing_html && !initial)
      {
        timeout_cursors(True);
        if (m_data->html_w == 0)
          html_create(frame,m_data);
        m_data->html_string = html_get_string(m_data);
        if (m_data->html_string == NULL)
          viewing_html = 0;
        else
          {
            XtUnmanageChild(m_data->text_w);
            XtManageChild(m_data->html_w);
            html_set_base_page(m_data,get_detach_dir());
            html_display(m_data);
          }
        timeout_cursors(False);
      }
    if (initial || !viewing_html)
      { 
        if (m_data->html_w != 0)
          XtUnmanageChild(m_data->html_w);
        XtManageChild(m_data->text_w);
...
FrameTypeName type;
{
    ...
    /* Phone-Tag text area */
    {
      m_data->phone_tag_text_widgets = XtVaCreateWidget("phone_tag_text_rc", xmRowColumnWidgetClass,pane,
          XmNorientation, XmVERTICAL,
          XmNmarginHeight, 0,
          XmNnumColumns, 2,
          XmNpacking, XmPACK_COLUMN,
        ...
        while (phone_tag_label_list[i] != NULL)
          {
            ...
            if (strcmp(phone_tag_label_list[i],"blank") == 0)
            ...
            else if (strcmp(phone_tag_label_list[i],"Date:") == 0)
              {
                ...
                sprintf(widget_name,"phone_tag_text_%d",i);
                m_data->phone_tag_text_widget[i] =
                  CreateLabeledTextSetWidth("phone_tag_time",date_time_widget,
                    phone_tag_label_list[i],
                    CLT_HORIZ|CLT_REPLACE_NL,40,phone_tag_label_length[i],
                    &label_widget);
                XtVaSetValues(m_data->phone_tag_text_widget[i], 
            ...
            else
              {
                ...
                m_data->phone_tag_text_widget[i] =
                  CreateLabeledTextSetWidth(
                    ...
                    phone_tag_label_list[i],
                    CLT_HORIZ|CLT_REPLACE_NL,75,phone_tag_label_length[i],
                    &label_widget);
                XtVaSetValues(m_data->phone_tag_text_widget[i], 
                XmNeditable, False, 
                NULL);
    ...
    ZmCallbackData cdata;
{
    ...
    if (!enable) XmToggleButtonSetState(toggle, False);
}
#endif /* !OZ_DATABASE */
...
Boolean required;
{
	...
	*ai = XtNew(AttachInfo);
	...
	if (type == FrameCompose) {
		...
		if (!enabled)
		    XmToggleButtonSetState((*ai)->autotype_w, False);
	    }
