...
/* Autologout timer */
#define TIMEOUT zync_timeout
extern int zync_timeout;
...
char *version = "7.8(79)";	/* version number of this server */
...
#include "linkage.c"
  ...
  zync_init(&argc, &argv);
  printf ("* %s %s Welcome to the Z-Mail Zync server, version %s\015\012",
	  t,host,ZYNC_VERSION);
  ...
  do {				/* command processing loop */
	...
	stream = NIL;
	printf("* BYE %s Zync server terminating connection\015\012",host);
	state = LOGOUT;
      }

				/* kludge for MacMS */
...

	...
	if ((!(anonymous || strcmp (cmd,"SELECT"))) ||
	  ...
	  if (s = snarf (&arg)){/* parse mailbox name */
	    ...
	    if (*arg == '"') v = snarf (&arg);
				/* parse message */
	    if (!(t = snarf (&arg))) response = misarg;
	    else if (arg) response = badarg;
	    else {		/* append the data */
	      STRING st;
	      INIT (&st,mail_string,(void *) t,strlen (t));
	      mail_append_full (NIL,s,u,v,&st);
	    }
	  }
